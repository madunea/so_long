...
/*   Updated: 2025/01/07 18:51:36 by marieduneau      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	char		*buffer;
	char		*line;
	static char	*line_after_n[MAX_FDS];
	...
	if (!buffer)
		return (NULL);
	line = get_full_line(fd, buffer, line_after_n[fd]);
	free(buffer);
	if (!line)
		return (NULL);
	line_after_n[fd] = extract_after_n(line);
	return (line);
}
